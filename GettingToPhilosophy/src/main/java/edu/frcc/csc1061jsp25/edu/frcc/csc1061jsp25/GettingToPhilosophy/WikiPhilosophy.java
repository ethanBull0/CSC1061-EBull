...
public class WikiPhilosophy {
	...
	public static void main(String[] args) throws IOException {
		...
		testConjecture(destination, source, 10, new ArrayList<String>());
	}
	 ...
	 * @throws IOException
	 */
	public static void testConjecture(String destination, String source, int limit, ArrayList<String> links)
			throws IOException {
		...
		for (Element para : paragraphs) {
			...
			for (Node node : iter) {
				...
				// It seems that element nodes specifically contain href, thus TextNode objects
				// are not useful here
				if (node instanceof Element) {
					...
					if (node.attributes().hasKey("href")) {
						if (node.attributes().get("href").charAt(0) != '#') {
							if (!(node.attributes().get("href").substring(0, 5).equals("https"))) {
								useMe += node.attributes().get("href");
								// check to see if it is a Help:IPA page
								if (useMe.contains("/wiki/") && !(useMe.contains("/wiki/File"))  && !(useMe.contains("/wiki/Help"))) {
								
									if (!(links.contains(useMe))) {

										System.out.println("Found page " + useMe);
										links.add(useMe);
										testConjecture(destination, useMe, limit - 1, links);
				 ...
				 * 
				 * 
				 * if (node instanceof TextNode) { //System.out.println("This node: ");
				 * //System.out.println(node.toString()); //System.out.println("This node END");
				 * } else if (node instanceof Element) {
				 * System.out.println("This element node: "); String nodeToString =
				 * node.toString(); System.out.println(nodeToString); if
				 * (nodeToString.contains("<a href=")) { String containsLink =
				 * nodeToString.substring(nodeToString.indexOf("<a href=") + 9); int start =
				 * nodeToString.indexOf("=\"") + 2; useMe += containsLink.substring(0,
				 * containsLink.indexOf(" ") - 1); System.out.println("This node END");
				 * System.out.println("Found page " + useMe); testConjecture(destination, useMe,
				 * 10); System.out.println("Couldn't find " + destination + "with depth " +
				 * limit + "."); } }
				 */
